$(`#form_input_jenis_informasi`).validate({
	rules: {
		name: {
			required: true,
			remote: {
				url: base_url + `jenisInformasi/isExist`,
				type: "post",
				data: {
					param: function () {
						return $("#param_jenis_informasi").val();
					},
				},
			},
		},
...
function editJenis(id) {
	_ajax(`${camelCase(menuActive)}/edit`, { id: id }, function (res) {
		if (res.status) {
			...
			$("#name_" + menuActive.toLowerCase()).val(res.res_data["name"]);
		} else {
			notif("failed");
		}
	});
