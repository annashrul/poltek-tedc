...
function load_select_category() {
	_ajax(base_controller+"master-konten/kategori/get_option", {type:"Kemahasiswaan"}, function (res) {
...
function add() {
	...
	$("#modal_title").text(`Tambah Kemahasiswaan`);
	$("#param").val("add");
...
function edit(id) {
	...
	_ajax(`edit`, { id: id }, function (res) {
		if (res.status) {
			$(`#modal_form`).modal("show");
			$("#modal_title").text(`Edit Kemahasiswaan`);
			$("#param").val("edit");
